...
%     KNEE_VEL_MAX_LIM = 400; %deg/s
    ...
    if FC %Use SLIP Embedding Controller
        ...
        %Calculate torque output usinddg PD controller
        [Knee_torque_command,Ankle_torque_command] = PDControl(...
            kp_knee,kd_knee,knee_des,knee_pos,dknee, ...
            kp_ankle,kd_ankle,ankle_des,ankle_pos,dankle);
    end
...
function [Knee_torque_command,Ankle_torque_command,deltaL,E] = SLIPControl(ankle, knee, hip, fc, la, ls, lt, ks, L0)
    ...
    J = [0;(-1).*lt.*(2.*fc.*cos(x(2)+x(3))+2.*ls.*sin(x(2))+la.*sin(x(2)+x(3))).*(4.*fc.^2+la.^2+4.*ls.^2+4.*lt.^2+8.*ls.*lt.*cos(x(2))+4.*la.*ls.*cos(x(3))+4.*la.*lt.*cos(x(2)+x(3))+(-8).*fc.*ls.*sin(x(3))+(-8).*fc.*lt.*sin(x(2)+x(3))).^(-1/2);(-1).*(4.*fc.^2+la.^2+4.*ls.^2+4.*lt.^2+8.*ls.*lt.*cos(x(2))+4.*la.*ls.*cos(x(3))+4.*la.*lt.*cos(x(2)+x(3))+(-8).*fc.*ls.*sin(x(3))+(-8).*fc.*lt.*sin(x(2)+x(3))).^(-1/2).*(2.*fc.*ls.*cos(x(3))+2.*fc.*lt.*cos(x(2)+x(3))+la.*ls.*sin(x(3))+la.*lt.*sin(x(2)+x(3)))];
    L  = (fc.^2+(1/4).*la.^2+ls.^2+lt.^2+2.*ls.*lt.*cos(x(2))+la.*ls.*cos(x(3))+la.*lt.*cos(x(2)+x(3))+(-2).*fc.*ls.*sin(x(3))+(-2).*fc.*lt.*sin(x(2)+x(3))).^(1/2);
    deltaL = L - L0;
    u = -ks*(deltaL)*J;
    E = 1/2*k*deltaL^2;
...
%     x(3) = ankle;
%     
%     KE = [(1/2).*(x(6).*((la.*Ms.*(la+ls.*cos(x(3))+lt.*cos(x(2)+x(3)))+(1/4).*lf.*Ms.*(lf+(-2).*ls.*sin(x(3))+(-2).*lt.*sin(x(2)+x(3)))).*x(4)+(la.*Ms.*(la+ls.*cos(x(3)))+(1/4).*lf.*Ms.*(lf+(-2).*ls.*sin(x(3)))).*x(5)+(la.^2.*Ms+(1/4).*lf.^2.*Ms).*x(6))+x(5).*(((1/2).*ls.*Ms.*((1/2).*ls+lt.*cos(x(2)))+Ms.*(la+ls.*cos(x(3))).*(la+ls.*cos(x(3))+lt.*cos(x(2)+x(3)))+(1/4).*Ms.*(lf+(-2).*ls.*sin(x(3))).*(lf+(-2).*ls.*sin(x(3))+(-2).*lt.*sin(x(2)+x(3)))).*x(4)+((1/4).*ls.^2.*Ms+Ms.*(la+ls.*cos(x(3))).^2+(1/4).*Ms.*(lf+(-2).*ls.*sin(x(3))).^2).*x(5)+(la.*Ms.*(la+ls.*cos(x(3)))+(1/4).*lf.*Ms.*(lf+(-2).*ls.*sin(x(3)))).*x(6))+x(4).*(((1/4).*lt.^2.*Mt+Ms.*((1/2).*ls+lt.*cos(x(2))).^2+Ms.*(la+ls.*cos(x(3))+lt.*cos(x(2)+x(3))).^2+lt.^2.*Ms.*sin(x(2)).^2+(1/4).*Ms.*(lf+(-2).*ls.*sin(x(3))+(-2).*lt.*sin(x(2)+x(3))).^2).*x(4)+((1/2).*ls.*Ms.*((1/2).*ls+lt.*cos(x(2)))+Ms.*(la+ls.*cos(x(3))).*(la+ls.*cos(x(3))+lt.*cos(x(2)+x(3)))+(1/4).*Ms.*(lf+(-2).*ls.*sin(x(3))).*(lf+(-2).*ls.*sin(x(3))+(-2).*lt.*sin(x(2)+x(3)))).*x(5)+(la.*Ms.*(la+ls.*cos(x(3))+lt.*cos(x(2)+x(3)))+(1/4).*lf.*Ms.*(lf+(-2).*ls.*sin(x(3))+(-2).*lt.*sin(x(2)+x(3)))).*x(6)))];
%     PE = (-1/2).*g.*(lt.*(2.*Ms+Mt).*cos(x(1))+ls.*Ms.*cos(x(1)+x(2)));
end

function u_KPBC = KPBC(x,k,E,Eref)
