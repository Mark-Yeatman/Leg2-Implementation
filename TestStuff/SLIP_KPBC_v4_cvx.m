function [Knee_torque_command, Ankle_torque_command, deltaL, hip_pos, t_out, dt]...
    ...
    FC, lf, la, ls, lt, k, d, kL0, hip_vel, knee_vel, ankle_vel)%, Eref)
...
%     KNEE_VEL_MAX_LIM = 400; %deg/s
    ...
    if FC %Use SLIP Embedding Controller
        ...
        x = zeros(6,1);
        x(1) =  hip_pos;
        x(2) = -knee_pos;         %reverse knee sign convention of biomechanics versus biped modeling
        x(3) =  ankle_pos;
        x(4) =  hip_vel;
        x(5) = -knee_vel;
        x(6) =  ankle_vel;
