...
mass = 91; %kg
axWC1 = subplot(2,1,1);
kneeW_data = level_walking_std.normal_cadence.knee.power;
...
plot(time,knee_joint_vel(I).*Knee_torque_command(I),time,kneeW_mean)
xlabel("Time (s)")
ylabel("Power (J/s)")
title("Knee")
legend("Pros","Winters")
...
axWC2 = subplot(2,1,2);
ankleW_data = level_walking_std.normal_cadence.ankle.power;
...
plot(time,ankle_joint_vel(I).*Ankle_torque_command(I),time,ankleW_mean)
xlabel("Time (s)")
ylabel("Power (J/s)")
title("Ankle")
legend("Pros","Winters")
